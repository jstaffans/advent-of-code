...
data Patch =
  Patch { id :: Int, x :: Int, y :: Int, width :: Int, height :: Int }
  deriving Show
...
patchBounds patches = go patches 0 0
  where
    go [] xb yb = (xb, yb)
    go ((Patch { x = x, y = y, width = w, height = h}):ps) xb yb = go ps (max xb (x + w)) (max yb (y + h))
...
toMatrix patches = runSTUArray $ do
  ...
  forM_ patches $ \p@Patch { x = x, y = y, width = w, height = h} -> do
    forM_ [y..(y+h-1)] $ \i -> do
      forM [x..(x+w-1)] $ \j -> do
...
part2 patches matrix =
  let patchSizes = map (\Patch{id = id, width = w, height = h} -> (id, w * h)) patches
